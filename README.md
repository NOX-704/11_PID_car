...
| 循迹阶梯差速 | 启用 | 10 ms 计算黑线重心并选择 1～4 档目标轮速 |
...
6. 初始化新的航向 PID。
7. `motor_init(3)` 初始化 TB6612FNG A/B 通道，启动 TIMG0 电机 PWM 和 TIMA0 10 ms 控制定时器。

运行阶段包含两条并行路径：

- 主循环：响应 10 ms 采样请求，通过 PB2/PB3 读取 MPU6050；每 500 ms 输出一次诊断数据。
- TIMA0 中断：读取八路 GPIO → 计算防抖阶梯差速 → 叠加 MPU6050 航向 PID → 计算编码器速度 → 执行双路速度 PI → 更新 PWM。
...
档位判定带有连续样本防抖：同一个“方向+档位”必须连续出现 `3` 个 10 ms 控制周期（约 30 ms）才会替换当前档位，确认期间继续执行原档位。直行也参与相同确认，避免黑线在中心附近抖动时反复切换左右差速。八路全黑停车立即生效；第 4 档后的八路全白仍保持最高档，并清除尚未完成的候选换档计数。
...
旧的 `TRACK_USE_STEERING_PID`、`Kp=45` 及其误差积分/微分实现已经全部删除。新的航向参数集中在 `user_driver/heading_pid.c`，默认 `Kp=2.0`、`Ki=0.0`、`Kd=1.0`，输出限幅 `±80 mm/s`，每 10 ms 最多变化 `6 mm/s`。
