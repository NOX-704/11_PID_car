...
旧的 `TRACK_USE_STEERING_PID`、`Kp=45` 及其误差积分/微分实现已经全部删除。新的航向参数集中在 `user_driver/heading_pid.c`，默认 `Kp=2.0`、`Ki=0.0`、`Kd=1.0`，输出限幅 `±80 mm/s`，每 5 ms 最多变化 `3 mm/s`，与原来每 10 ms 变化 `6 mm/s` 的斜率相同。
