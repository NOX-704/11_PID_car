...
基于 TI MSPM0G3507 的双直流电机循迹小车工程。当前版本使用八路独立 GPIO 读取亚博智能八路循迹模块，由四档阶梯差速提供主要转向，再使用 PCB I²C1 上平放锁死的 MPU6050 做航向 PID，最后由双轮编码器速度 PI 驱动 TB6612FNG。

> 旧的纯循迹转向 PID 和旧速度环 D 项已经删除。当前只有一套 MPU6050 航向 PID；电机底层使用独立的双轮速度 PI。
...
|---|---|---|
| H1/H6/H7 | MPU6050 等外设共享 I²C1 | PB2=SCL，PB3=SDA |
...
| 功能 | 状态 | 当前实际行为 |
|---|---|---|
| 八路循迹输入 | 已改为 GPIO | 亚博智能八路循迹模块：`1=黑线`，`0=白色` |
| 循迹阶梯差速 | 启用 | 5 ms（200 Hz）计算黑线重心并选择 1～4 档目标轮速 |
| MPU6050 航向 PID | 启用 | 直线锁航向；顺时针半圆按 `ω=v/0.5m` 跟踪目标角速度 |
| 双路电机 PWM | 启用 | TIMG0 双通道，约 10 kHz，比较值范围 `0~4000` |
| 双编码器测速 | 启用 | 每个编码器只统计 A 相上升沿，10 ms 计算一次速度 |
| 双路增量式速度 PI | 启用 | `Kp=0.5`、`Ki=0.4`，不使用编码器微分项 |
| UART 调试输出 | TX 启用 | 每 500 ms 输出八路逻辑、IMU、yaw、gyroZ、模式和修正量 |
...
程序初始化后会立即启动 5 ms 总控制定时器，没有独立的开始/停止状态机。首次上电调试必须架空车轮。
...
4. 把两路目标速度清零。
5. 车辆保持静止，初始化 MPU6050 并用 200 个样本进行约 2 秒 Z 轴零偏校准。
6. 初始化新的航向 PID。
7. `motor_init(3)` 初始化 TB6612FNG A/B 通道，启动 TIMG0 电机 PWM 和 TIMA0 5 ms 总控制定时器。

运行阶段包含两条并行路径：

- 主循环：响应每两个控制节拍产生的 10 ms 请求，通过 PB2/PB3 读取 MPU6050；每 500 ms 输出一次诊断数据。
- TIMA0 每个 5 ms 中断：读取八路 GPIO → 计算防抖阶梯差速 → 叠加 MPU6050 航向 PID。
- TIMA0 每两个中断：计算编码器速度 → 执行双路速度 PI → 更新 PWM，继续保持 100 Hz。
- K230 UART3 由 SysConfig 完成引脚和波特率初始化，但本工程暂未启用接收中断或协议解析。

主循环不再使用 `delay_ms(500)`。阻塞式 I²C 读取不放入中断，避免 MPU6050 接线或总线异常卡死电机控制。
...
当前读取层不再进行左右镜像，软件逻辑位置直接对应 PCB 网络：
...
串口输出和差速权重均保持 `L1 L2 L3 L4 R1 R2 R3 R4` 的车体左到右顺序；SysConfig 和 PCB 引脚本身没有改动。

当前使用亚博智能八路循迹模块。PCB 的 H2 接口使用模块的 8 路并行 GPIO 输出，不是 UART 帧或 I²C 数据。亚博官方资料定义模块原始数字量为低电平有效：

| 引脚原始电平 | 表面 | `huidu_value[]` | 循迹含义 |
|---|---|---:|---|
| 高电平 `1` | 白色背景、指示灯灭 | `0` | 未检测到黑线 |
| 低电平 `0` | 黑色胶带、指示灯亮 | `1` | 检测到黑线 |

`huidu_get_value()` 在读取层完成反相，使控制层始终使用“黑=`1`、白=`0`”的统一语义。因此模块原始全白为 `11111111`，转换后的 `huidu_value[]` 为 `00000000`。若全白前处于第 4 档最高差速，控制函数不写入任何新目标速度，直接保留上一周期的左右轮目标，直到任意探头重新识别到黑线；第 1～3 档和直行状态遇到全白则按直行处理。

模块其他通信方式的官方格式如下，但当前 PCB 工程未使用：

- UART 数字帧：ASCII 文本 `$D,x1:1,x2:0,...,x8:0#`；数字输出请求命令为 `$0,0,1#`。
- I²C 数字量：读取寄存器 `0x30`，bit7～bit0 依次对应 X1～X8。
- 三种方式的数字语义一致：`1` 表示白底，`0` 表示黑线。

### GPIO 接线

| 外设功能 | 芯片/模块型号 | 地猛星引脚 | IOMUX 索引 | 片上复用功能 | 备注 |
|---|---|---|---|---|---|
| 循迹 L1 / 最左 / H2-3 | 亚博智能八路循迹模块 | PA18 | PINCM40 | GPIO 输入 | 原始黑=`0`、白=`1`；软件转换后黑=`1` |
| 循迹 L2 / H2-4 | 同上 | PA16 | PINCM38 | GPIO 输入 | 原始黑=`0`、白=`1`；软件转换后黑=`1` |
| 循迹 L3 / H2-5 | 同上 | PB7 | PINCM24 | GPIO 输入 | 原始黑=`0`、白=`1`；软件转换后黑=`1` |
| 循迹 L4 / 左中 / H2-6 | 同上 | PA17 | PINCM39 | GPIO 输入 | 原始黑=`0`、白=`1`；软件转换后黑=`1` |
| 循迹 R1 / 右中 | 同上 | PA21 | PINCM46 | GPIO 输入 | 原始黑=`0`、白=`1`；PA21 兼有 VREF- 相关功能 |
| 循迹 R2 | 同上 | PA22 | PINCM47 | GPIO 输入 | 原始黑=`0`、白=`1`；软件转换后黑=`1` |
| 循迹 R3 | 同上 | PA24 | PINCM54 | GPIO 输入 | 原始黑=`0`、白=`1`；软件转换后黑=`1` |
| 循迹 R4 / 最右 / H2-10 | 同上 | PA2 | PINCM7 | GPIO 输入 | 原始黑=`0`、白=`1`；按 PCB 配置，须确认核心板确实引出 PA2 |
...
算法只把值为 `1`、检测到黑线的探头计入重心。加权结果为负表示黑带偏左，为正表示黑带偏右，绝对值越大表示偏得越远。多路同时压到胶带时使用平均偏移选择档位：

| 平均偏移绝对值 | 差速档位 | 弯道内侧轮 | 弯道外侧轮 |
|---:|---:|---:|---:|
| 居中 | 直行 | `380` | `380` |
| `0~1` | 1 | `350` | `410` |
| `1~2` | 2 | `300` | `460` |
...
四档都满足 `内侧轮+外侧轮=760`，因此差速车理论中心速度 `(左轮+右轮)/2` 始终约为 `380 mm/s`。档位升高时，内侧轮逐档减速、外侧轮逐档加速，主要改变车辆角速度而不改变重心前进速度。电机 1/A 通道是右轮，电机 2/B 通道是左轮；黑线偏左时右轮为外侧轮，黑线偏右时左轮为外侧轮。
...
- 转向不足：在保持该档内外轮之和不变的前提下，提高对应 `TRACK_OUTER_SPEED_n` 并等量降低 `TRACK_INNER_SPEED_n`。
- 转向太猛：降低对应外侧轮速度，并等量提高该档内侧轮速度。
- 整车速度太快：降低 `TRACK_STRAIGHT_SPEED`，并让每档满足 `INNER_n + OUTER_n = 2 × TRACK_STRAIGHT_SPEED`。
- 档位仍容易抖动：增大 `TRACK_GEAR_CONFIRM_TICKS`；转向响应太慢则减小，建议范围 `2~5`。
- 第 4 档后八路全白会持续保持最高档原运动状态，重新识别到黑线后才退出保持。

旧的 `TRACK_USE_STEERING_PID`、`Kp=45` 及其误差积分/微分实现已经全部删除。新的航向参数集中在 `user_driver/heading_pid.c`，当前 `Kp=1.0`、`Ki=0.0`、`Kd=2.0`，输出限幅 `±80 mm/s`，每 5 ms 最多变化 `3 mm/s`，与原来每 10 ms 变化 `6 mm/s` 的斜率相同。
...
| 灰度循迹/航向 PID 周期 | `5 ms / 200 Hz` |
| 编码器测速/速度 PI 周期 | `10 ms / 100 Hz` |
| 速度 PI `Kp` | `0.5` |
| 速度 PI `Ki` | `0.4` |
| 航向 PID `Kp` | `1.0` |
| 航向 PID `Ki` | `0.0` |
...
MPU6050 平放且芯片正面朝上时，驱动默认把原始 Z 轴角速度乘 `-1`，统一成顺时针为正。上电后必须保持车体静止约 2 秒。若串口中手动顺时针转动车体时 `GZ` 仍为负，应把 `mpu6050.c` 的 `MPU6050_CLOCKWISE_SIGN` 改成 `+1.0f`。

直线模式把进入直线时的 yaw 锁为目标；顺时针半圆模式根据当前平均目标轮速和 `500 mm` 半径生成目标角速度。曲线结束并稳定居中 200 ms 后重新锁定下一段直线航向。由于 MPU6050 没有磁力计，yaw 仅用于单段直线和半圆的短时相对控制，不能作为全场绝对方向。
...
| MPU6050 SCL / H1、H6、H7 第3脚 | MPU6050 | PB2 | PINCM15 | I2C1_SCL | 400 kHz，模块侧需有 3.3 V 上拉 |
| MPU6050 SDA / H1、H6、H7 第4脚 | MPU6050 | PB3 | PINCM16 | I2C1_SDA | AD0 接地时地址 `0x68` |
...
- 当前模块为亚博智能八路循迹模块，但具体供电和输出电平版本未记录。若模块使用 5 V 供电，必须确认 8 路数字输出的高电平不超过 3.3 V，必要时使用电平转换。
...
```text
11_PID_car/
├── main.c                         # 初始化、MPU采样调度和串口诊断
├── empty.syscfg                   # 时钟、引脚和外设配置源
├── user_driver/
│   ├── delay.c/.h                 # 阻塞毫秒延时
│   ├── heading_pid.c/.h           # 直线/半圆 MPU6050 航向 PID
│   ├── huidu.c/.h                 # 八路 GPIO、阶梯差速和航向修正叠加
│   ├── key.c/.h                   # 当前只保留编码器 GPIOB 中断计数
│   ├── motor.c/.h                 # TB6612FNG、测速和双路增量式速度 PI
│   ├── mpu6050.c/.h               # PB2/PB3 I2C1、校零、gyroZ与yaw
...
主循环每 500 ms 输出一行 8 个数字。黑线处在两个中心探头之间时的典型输出为：

```text
00000000
```

数字顺序是 `L1 L2 L3 L4 R1 R2 R3 R4`；其中 `1` 表示检测到黑色胶带，`0` 表示白色背景。例如 `00010000` 表示 L4 检测到黑线，控制器会轻微向左修正。
...
2. 把各探头分别放在白底和黑带上，确认串口对应输出为 `0` 和 `1`。
...
- 亚博智能八路循迹模块的具体供电和输出电平版本没有记录，接线前仍需确认输出高电平不超过 3.3 V。
- `00000000` 既可能表示黑带正确位于 L4/R1 间隙，也可能表示车辆完全离开黑带进入纯白区域；上一状态为第 4 档时会无限保持原运动状态，只有重新检测到黑线或八路全黑才会退出，因此实车失线后可能持续转向。
- 当前四档差速是离散控制，档位速度仍需根据传感器离地高度、轮胎摩擦和实际赛道继续整定。
- 八路全部检测到黑线（`11111111`）时无法判断胶带中心，代码会把两路目标速度设为 `0`。
- SW1 / PB6 已配置为启动按键输入，但开始/停止状态机尚未实现，上电后控制定时器仍会立即运行。
- 编码器只统计 A 相上升沿，无法判断方向，也没有利用 B 相提高分辨率。
- 航向 PID、速度 PI、轮径、编码器计数常量和目标速度均写死在源码中，没有在线调参或掉电保存。
- MPU6050 没有磁力计，yaw 会漂移；代码只在每段直线/半圆内短时使用，并在状态切换时重新锁定。
