...
基于 TI MSPM0G3507 的双直流电机循迹小车工程。当前版本使用八路独立 GPIO 读取数字循迹信号，对不亮探头计算黑带横向重心并生成两路目标轮速，再利用编码器反馈和增量式 PID 调节 TB6612FNG 的 A/B 两个电机通道。
...
| 循迹决策 | 启用 | 10 ms 计算不亮探头的加权重心，并立即产生完整目标轮速差 |
...
直行时黑色胶带位于 L4 和 R1 两个中心探头之间，八路探头都照到白色并全部亮灯，通常会把 `11111111` 判为直行。若全亮前刚处于第 3/4 档严重偏移，则继续保持原差速方向，直到任意一路再次熄灭，避免车辆越过黑带后突然改为直行。
...
算法只把值为 `0` 的不亮探头计入重心。加权结果为负表示黑带偏左，为正表示黑带偏右，绝对值越大表示偏得越远。多路同时压到胶带时取整体平均位置，不再依赖旧版 `if/else` 的匹配顺序。

差速继续使用当前已验证的参数：

| 典型亮灭状态 `L1…R4` | 黑带状态 | 电机 1 / A / 右轮目标 | 电机 2 / B / 左轮目标 | 目标差速 |
|---|---|---:|---:|---:|
| `11111111` | 全亮，上一状态不是第 3/4 档 | `200` | `200` | `0` |
| `11111111` | 全亮，上一状态是第 3/4 档 | 保持上一目标 | 保持上一目标 | 保持 |
| 左右不亮探头重心对称 | 黑带重心居中 | `200` | `200` | `0` |
| `11101111` | L4 不亮，左偏一档 | `220` | `150` | `70` |
| `11011111` | L3 不亮，左偏二档 | `260` | `150` | `110` |
| `10111111` | L2 不亮，左偏三档 | `320` | `150` | `170` |
| `01111111` | L1 不亮，左偏四档 | `420` | `150` | `270` |
| `11110111` | R1 不亮，右偏一档 | `150` | `220` | `70` |
| `11111011` | R2 不亮，右偏二档 | `150` | `260` | `110` |
| `11111101` | R3 不亮，右偏三档 | `150` | `320` | `170` |
| `11111110` | R4 不亮，右偏四档 | `150` | `420` | `270` |
| `00000000` | 全灭，无法判断中心 | `0` | `0` | `0` |
...
每次差速档位变化时，完整目标轮速差会立即送入增量式 PID，避免车辆短暂压到最外端时差速还没有建立。PID 算出的实际 PWM 在输出前限制为每个 10 ms 周期最多变化 `200`；持续请求从 `0` 变到满量程 `4000` 时需要 `20 × 10 ms = 200 ms`，较小变化按相同斜率更早完成。这样既保留大差速响应，也避免 PWM 阶跃。

TIMG0 当前使用 `EDGE_ALIGN_UP`，其捕获比较值与实际高电平占空比方向相反。因此代码内部统一使用“逻辑 PWM duty”，即 `0=停止`、`4000=满占空比`，写入硬件前转换为 `compare = 4000 - duty`。如果直接把逻辑 duty 写进比较寄存器，就会出现数值越大、实际占空比反而越小的问题。
