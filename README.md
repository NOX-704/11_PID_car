...
电机 1/A 通道是右轮，电机 2/B 通道是左轮。黑线偏左时右轮为弯道外侧轮，右轮加速、左轮保持 `150`；黑线偏右时反向分配。

档位判定带有连续样本防抖：同一个“方向+档位”默认必须连续出现 `3` 个 5 ms 循迹周期（约 15 ms）才会替换当前档位，确认期间继续执行原档位。直行也参与相同确认，避免黑线在中心附近抖动时反复切换左右差速。八路全黑停车立即生效；第 4 档后的八路全白仍保持最高档，并清除尚未完成的候选换档计数。

实际 PWM 由 `motor.c` 的速度 PI 和每 10 ms 最大变化 `80` 的输出斜坡控制，因此档位切换和航向修正不会直接形成 PWM 满幅阶跃。

阶梯参数集中在 `user_driver/huidu.c` 顶部：

- 转向不足：提高相应的 `TRACK_OUTER_SPEED_1~4`，或降低 `TRACK_INNER_SPEED`。
- 转向太猛：降低对应外侧轮速度。
- 整车速度太快：同时降低直行、内侧和四档外侧速度。
