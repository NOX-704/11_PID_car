...
基于 TI MSPM0G3507 的双直流电机循迹小车工程。当前版本使用八路独立 GPIO 读取亚博智能八路循迹模块，对检测到黑线的探头计算横向重心，由四档阶梯差速生成两路目标轮速，再利用编码器反馈和增量式 PID 调节 TB6612FNG 的 A/B 两个电机通道。

> 循迹转向 PID 当前通过编译开关禁用，外环使用四档阶梯差速；两路编码器速度 PID 仍然启用。
...
| 循迹转向 PID | 已禁用并保留 | `TRACK_USE_STEERING_PID=0U`；改为 `1U` 可重新编译启用 |
...
- 主循环：读取八路 GPIO，拼成 8 位黑线检测状态字符串，通过 UART0 每 500 ms 发送一次。
- TIMA0 中断：读取八路 GPIO → 计算黑线重心和阶梯差速 → 更新两路目标速度 → 计算编码器速度 → 执行双路增量式速度 PID → 更新 PWM。
...
实际 PWM 仍由 `motor.c` 的速度 PID 和每 10 ms 最大变化 `30` 的输出斜坡控制，因此档位切换不会直接形成 PWM 满幅阶跃。
...
连续转向 PID 的参数和实现没有删除，而是放在 `#if TRACK_USE_STEERING_PID` 中。当前开关为 `0U`；以后若设为 `1U`，会恢复保留的 PID 版本，其中包含用户最近手动设置的 `Kp=45`。
