...
| 循迹决策 | 启用 | 10 ms 计算不亮探头的加权重心，目标速度用 200 ms 斜坡切换 |
...
| 黑带状态 | 电机 1 / A / 右轮目标速度 | 电机 2 / B / 左轮目标速度 |
|---|---:|---:|
| 8 路全亮，且上一状态不是第 3/4 档 | `150` | `150` |
| 8 路全亮，且上一状态是第 3/4 档 | 保持上一目标 | 保持上一目标 |
| 不亮探头重心左右对称 | `150` | `150` |
| 黑带在 L4 一档 | `250` | `150` |
| 黑带在 L3 二档 | `300` | `150` |
| 黑带在 L2 三档 | `350` | `150` |
...
每次差速档位变化时，代码从当前目标轮速出发，在 `20 × 10 ms = 200 ms` 内线性过渡到新档位。平滑的是增量式 PID 的目标轮速，因此编码器闭环继续有效，PID 输出的 PWM 也不会再直接面对阶梯突变。如果 200 ms 内再次换档，斜坡会从当时的目标轮速重新开始，不会跳变。
...
- 循迹使用黑带加权重心和四档差速，不是连续横向 PID；`150/250/300/350/400` 为当前参数，仍可能需要继续整定。
