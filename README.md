...
当前使用亚博智能八路循迹模块。每路 GPIO 在 SysConfig 中配置为数字输入和内部下拉；`huidu_get_value()` 直接把模块数字输出转换为以下统一语义：
...
直行时黑色胶带位于 L4 和 R1 两个中心探头之间，八路探头都照到白色，通常会把 `00000000` 判为直行。若全白前处于第 3/4 档严重偏移，则保持原阶梯差速最多 150 ms，随后恢复直行；这样既能在刚越过黑线时继续找线，也不会永久锁在单边大差速状态。

### GPIO 接线

| 外设功能 | 芯片/模块型号 | 地猛星引脚 | IOMUX 索引 | 片上复用功能 | 备注 |
|---|---|---|---|---|---|
| 循迹 L1 / 最左 / H2-3 | 亚博智能八路循迹模块 | PA18 | PINCM40 | GPIO 输入 | 内部下拉；黑=`1`，白=`0` |
| 循迹 L2 / H2-4 | 同上 | PA16 | PINCM38 | GPIO 输入 | 内部下拉；黑=`1`，白=`0` |
| 循迹 L3 / H2-5 | 同上 | PB7 | PINCM24 | GPIO 输入 | 内部下拉；黑=`1`，白=`0` |
| 循迹 L4 / 左中 / H2-6 | 同上 | PA17 | PINCM39 | GPIO 输入 | 内部下拉；黑=`1`，白=`0` |
| 循迹 R1 / 右中 | 同上 | PA21 | PINCM46 | GPIO 输入 | 黑=`1`，白=`0`；PA21 兼有 VREF- 相关功能 |
| 循迹 R2 | 同上 | PA22 | PINCM47 | GPIO 输入 | 内部下拉；黑=`1`，白=`0` |
| 循迹 R3 | 同上 | PA24 | PINCM54 | GPIO 输入 | 内部下拉；黑=`1`，白=`0` |
| 循迹 R4 / 最右 / H2-10 | 同上 | PA2 | PINCM7 | GPIO 输入 | 内部下拉；黑=`1`，白=`0`；按 PCB 配置，须确认核心板确实引出 PA2 |
...
- 第 3/4 档后八路全白只保持旧指令 150 ms，超时自动恢复直行。
...
- `00000000` 既可能表示黑带正确位于 L4/R1 间隙，也可能表示车辆完全离开黑带进入纯白区域；代码只在上一状态为第 3/4 档时保持搜索方向 150 ms，随后恢复直行，因此不能保证所有高速丢线场景都能重新找回黑线。
