...
实车确认亚博模块的安装方向与 PCB 的 `XUNJI_L1…R4` 网络命名左右相反，读取层已经做空间镜像：
...
因此串口输出和 PID 权重仍然保持 `L1 L2 L3 L4 R1 R2 R3 R4` 的车体左到右顺序；SysConfig 和 PCB 引脚本身没有改动。
...
电机 1/A 通道是右轮，电机 2/B 通道是左轮。黑线偏左时右轮为弯道外侧轮，右轮加速、左轮保持 `150`；黑线偏右时反向分配。实际 PWM 仍由 `motor.c` 的速度 PID 和每 10 ms 最大变化 `200` 的输出斜坡控制，因此档位切换不会直接形成 PWM 满幅阶跃。
